...
class FlowBatchPilot {
  ...
  initializeElements() {
    ...
    this.addImagesInput = document.getElementById('addImagesInput');
    this.addImagesBtn = document.getElementById('addImagesBtn');
    
    // Preview Overlay
    this.previewOverlay = document.getElementById('previewOverlay');
    this.previewImage = this.previewOverlay ? this.previewOverlay.querySelector('img') : null;
  ...
  attachEventListeners() {
    ...
    this.promptInput.addEventListener('input', () => {
      // Sync global prompt input to per-image prompts
      const lines = this.promptInput.value.split(/\r?\n/);
      // Update promptsPerImage based on current files count
      if (this.currentFiles.length > 0) {
        this.promptsPerImage = this.currentFiles.map((_, i) => lines[i] || '');
        this.renderGallery(); // Update gallery inputs
      }
    ...
    this.videoRatio.addEventListener('change', () => this.saveUIState());
    ...
    if (this.addImagesBtn && this.addImagesInput) {
      this.addImagesBtn.addEventListener('click', () => this.addImagesInput.click());
      this.addImagesInput.addEventListener('change', (e) => this.handleAddImages(e));
    }
  ...
  restoreUIState(uiState) {
    ...
    // Restore video ratio
    if (uiState.videoRatio) {
      this.videoRatio.value = uiState.videoRatio;
    }
  ...
  async saveUIState() {
    try {
      const uiState = {
        ...
        videoRatio: this.videoRatio.value,
  ...
  handleFolderSelection(event) {
    ...
    const imgs = files.filter(f => (f.type && f.type.startsWith('image/')) || /\.(png|jpe?g|webp|gif|bmp)$/i.test(f.name));
    this.currentFiles = this.sortFilesByName(imgs);

    this.folderText.textContent = `${imgs.length} 个文件已选择`;
    this.folderLabel.classList.add('has-files');

    this.addLogEntry(`已选择 ${imgs.length} 个图片文件`, 'success');
    this.updatePreview();
    this.tryLoadPromptTxt(files).catch(() => {});
    this.renderGallery();
  ...
  async tryLoadPromptTxt(files) {
    const txt = files.find(f => {
      const name = (f.name || '').trim().toLowerCase();
      const rel = (f.webkitRelativePath || '').trim().toLowerCase();
      return name === 'prompt.txt' || rel.endsWith('/prompt.txt') || rel.endsWith('\\prompt.txt');
    });
    if (!txt) return;
    try {
      ...
      this.promptsPerImage = this.currentFiles.map((f, idx) => lines[idx] || '');
      
      // Sync to global input
      this.promptInput.value = lines.join('\n');
      
      this.addLogEntry(`已读取 prompt.txt，共 ${lines.length} 行`, 'success');
      this.updatePreview();
      this.renderGallery();
      this.saveUIState();
    } catch (e) {
      this.addLogEntry(`读取 prompt.txt 失败: ${e.message}`, 'warning');
    }
  }
  ...
  renderGallery() {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const promptVal = (this.promptsPerImage && this.promptsPerImage[i]) || '';
      const card = document.createElement('div');
      card.className = 'thumb';
      card.innerHTML = `
        <div class="thumb-img"><img src="${url}" alt="${file.name}"></div>
        <div class="thumb-body">
          <div style="font-size:11px; color:#9ca3af;">${file.name}</div>
          <input class="input" data-index="${i}" placeholder="提示词..." value="${promptVal.replace(/"/g, '&quot;')}">
          <div class="thumb-actions">
            <button class="btn btn-secondary" data-action="remove" data-index="${i}" style="padding:6px 8px; font-size:12px;">删除</button>
          </div>
        </div>
      `;
      ...
      // Hover Preview
      const imgDiv = card.querySelector('.thumb-img');
      imgDiv.addEventListener('mouseenter', () => this.showPreview(url));
      imgDiv.addEventListener('mouseleave', () => this.hidePreview());

      const input = card.querySelector('input.input');
      input.addEventListener('input', (e) => {
        const idx = parseInt(e.target.dataset.index, 10);
        this.promptsPerImage[idx] = e.target.value;
        
        // Sync back to global input
        this.promptInput.value = this.promptsPerImage.join('\n');
      ...
      removeBtn.addEventListener('click', () => {
        ...
        if (this.promptsPerImage && this.promptsPerImage.length) {
          this.promptsPerImage.splice(idx, 1);
        }
        // Sync back to global input after removal
        this.promptInput.value = this.promptsPerImage.join('\n');
  ...
  handleAddImages(event) {
    ...
    this.currentFiles.push(...list);
    const start = (this.promptsPerImage && this.promptsPerImage.length) ? this.promptsPerImage.length : 0;
    for (let i = 0; i < list.length; i++) {
      this.promptsPerImage[start + i] = '';
    }
    this.addLogEntry(`已添加图片: ${list.length} 张`, 'success');
    this.renderGallery();
    this.updatePreview();
    this.saveUIState();
  ...
  async preloadQueueFiles(taskCount, queueId) {
    ...
    for (let i = 0; i < taskCount; i++) {
      ...
      const entry = await this.getCompressedFileEntry(file, i);
  ...
  async handleFileRequest(fileIndex, sendResponse) {
    ...
    try {
      ...
      if (!fileEntry) {
        ...
        fileEntry = await this.getCompressedFileEntry(file, fileIndex);
  ...
  async imageFileToWebPBase64(file, quality = 0.85, maxWidth = 2048, maxHeight = 2048) {
    try {
      ...
      return await this.arrayBufferToBase64(buf);
    }
  }

  async getCompressedFileEntry(file, index) {
    let base64Data;
    let type = file.type;
    let name = file.name;
    if (file.type && file.type.startsWith('image/')) {
      base64Data = await this.imageFileToWebPBase64(file, 0.85);
      type = 'image/webp';
      ...
      const buf = await this.fileToArrayBuffer(file);
      base64Data = await this.arrayBufferToBase64(buf);
    }
    return { index, name, type, size: file.size, base64Data };
  }
  ...
  updatePreview() {
    ...
    if (prompts.length === 0 && this.currentFiles.length === 0) {
      this.previewBody.innerHTML = `
        <tr>
          <td colspan="3" class="preview-empty">请输入提示词并选择图片文件夹</td>
        </tr>
      `;
      return;
    }
    ...
    for (let i = 0; i < taskCount; i++) {
      ...
      html += `
        <tr>
          ...
          <td class="task-filename">${file.name}</td>
        </tr>
      `;
    }
    ...
    if (prompts.length !== this.currentFiles.length) {
      html += `
        <tr>
          <td colspan="3" class="preview-empty" style="color: var(--warning);">
            ⚠️ 数量不匹配: ${prompts.length} 个提示词 vs ${this.currentFiles.length} 张图片
          </td>
        </tr>
      `;
    }
  ...
  getPrompts() {
    if (this.promptsPerImage && this.promptsPerImage.length) {
      return this.currentFiles.map((_, idx) => (this.promptsPerImage[idx] || '').trim());
    }
  ...
  async startQueue() {
    try {
      ...
      const queueState = {
        ...
        videoRatio: metadata.videoRatio,
  ...
  addLogEntry(message, type = 'info') {
    ...
    if (this.logEntries) {
      this.logEntries.push(`[${timestamp}] [${type.toUpperCase()}] ${message}`);
    }
  ...
  exportLogs() {
    if (!this.logEntries || this.logEntries.length === 0) {
      this.addLogEntry('没有日志可导出', 'warning');
      return;
    }
    ...
    a.href = url;
    a.download = `flow-batch-logs-${new Date().toISOString().replace(/[:.]/g, '-')}.txt`;
